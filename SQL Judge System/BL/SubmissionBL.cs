...
namespace SQL_Judge_System.BL
{
    internal class SubmissionBL
    {
        ...
        public static DataTable GetStudentSubmissionMetrics(int studentID, DateTime startDate, DateTime endDate)
        {
            ...
            {
                // Returns a safe, empty schema template structure to prevent downstream application crashes
                DataTable emptyTable = new DataTable();
                emptyTable.Columns.Add("TotalSubmissions", typeof(int));
                emptyTable.Columns.Add("HighestPoints", typeof(int));
                emptyTable.Columns.Add("AveragePoints", typeof(double));
                emptyTable.Columns.Add("SuccessRate", typeof(double));
                return emptyTable;
            }

            return SubmissionDL.GetStudentSubmissionMetrics(studentID, startDate, endDate);
        }
        public static int GetContestRegisteredStudents(DateTime startDate, DateTime endDate)
        {
            ...
            return SubmissionDL.GetContestRegisteredStudents(startDate, endDate);
        }
        ...
        public static int GetRunTimeErrorCount(DateTime startDate, DateTime endDate)
        {
            if (startDate > endDate) return 0;

            return SubmissionDL.RunTimeErrorCount(startDate, endDate);
        }
        public static int GetPendingStatusCount(DateTime startDate, DateTime endDate)
        {
            if (startDate > endDate) return 0;

            return SubmissionDL.PendingStatusCount(startDate, endDate);
        }
        public static int GetCorrectSubCount(DateTime startDate, DateTime endDate)
        {
            if (startDate > endDate) return 0;

            return SubmissionDL.CorrectSubCount(startDate, endDate);
        }
        public static int GetHighestScore(DateTime startDate, DateTime endDate)
        {
            if (startDate > endDate) return 0;

            return SubmissionDL.HighestScore(startDate, endDate);
        }
        public static double GetAverageScore(DateTime startDate, DateTime endDate)
        {
            if (startDate > endDate) return 0.0;

            return SubmissionDL.AverageScore(startDate, endDate);
        }
        public static double GetOverallCompletionRate(DateTime startDate, DateTime endDate)
        {
            if (startDate > endDate) return 0.0;

            return SubmissionDL.OverallCompletionRate(startDate, endDate);
        }
    }
}
